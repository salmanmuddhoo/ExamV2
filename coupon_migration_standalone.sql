-- =====================================================
-- STANDALONE COUPON CODE MIGRATION
...
-- Now create the tables and other objects
CREATE TABLE IF NOT EXISTS public.coupon_codes (
    ...
    applicable_tiers UUID[] NOT NULL DEFAULT '{}',
...
CREATE INDEX IF NOT EXISTS idx_coupon_codes_code ON public.coupon_codes(UPPER(code));
...
CREATE INDEX IF NOT EXISTS idx_coupon_codes_created_at ON public.coupon_codes(created_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_coupon_usages_payment_transaction_id ON public.coupon_usages(payment_transaction_id);
...
DROP POLICY IF EXISTS "Admins can view all coupon codes" ON public.coupon_codes;
DROP POLICY IF EXISTS "Admins can create coupon codes" ON public.coupon_codes;
DROP POLICY IF EXISTS "Admins can update coupon codes" ON public.coupon_codes;
DROP POLICY IF EXISTS "Admins can delete coupon codes" ON public.coupon_codes;
...
DROP POLICY IF EXISTS "Authenticated users can create coupon usages" ON public.coupon_usages;
...
-- Create RLS policies for coupon_usages
...
BEGIN
    ...
    INSERT INTO public.coupon_usages (
        coupon_id, user_id, payment_transaction_id,
        discount_amount, original_amount, final_amount, currency
    ) VALUES (
        v_validation.coupon_id, v_payment_transaction.user_id, p_payment_transaction_id,
        v_discount_amount, p_original_amount, v_final_amount, p_currency
    );
...
SELECT
    cc.id, cc.code, cc.description, cc.discount_percentage,
...
INSERT INTO public.coupon_codes (
    ...
    applicable_tiers, applicable_billing_cycles
) VALUES (
    'WELCOME10',
...
-- Done!
SELECT 'Coupon system migration completed successfully!' as message;

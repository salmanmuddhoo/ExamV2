-- Fix expire_yearly_subscriptions() function to handle duplicate user_ids
-- Resolves error: "ON CONFLICT DO UPDATE command cannot affect row a second time"
-- This occurs when a user has multiple expired yearly subscriptions
...
BEGIN
  ...
  -- Use DISTINCT ON to ensure each user_id is only processed once
...
COMMENT ON FUNCTION expire_yearly_subscriptions IS
  'Expires yearly subscriptions when they reach their subscription_end_date (1 year after purchase) and downgrades to free tier. Uses DISTINCT ON to handle cases where a user has multiple expired subscriptions.';

-- Update check_user_subscription_access to use token_limit_override for carryover tokens
...
BEGIN
  ...
  -- Calculate actual token limit (use override if set, otherwise use tier limit)
  v_actual_token_limit := COALESCE(v_subscription.token_limit_override, v_tier.token_limit);

  -- Calculate remaining tokens
  IF v_actual_token_limit IS NULL THEN
    v_tokens_remaining := -1; -- Unlimited
  ELSE
    v_tokens_remaining := v_actual_token_limit - v_subscription.tokens_used_current_period;
  END IF;
...
-- Comment
COMMENT ON FUNCTION check_user_subscription_access IS
  'Checks user subscription access for features. Uses token_limit_override if set (for carryover tokens), otherwise uses tier token_limit.';

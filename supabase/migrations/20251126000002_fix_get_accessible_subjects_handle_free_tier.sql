-- Fix get_accessible_subjects_for_user to properly handle FREE tier users
--
-- Issue: Current RPC treats all non-admin users as students with subscription filtering
-- This incorrectly filters FREE tier users who should have access to ALL subjects
--
-- Fix: Check subscription tier and only filter for student/student_lite tiers
-- Pro and Free tiers get access to all active subjects
...
BEGIN
  ...
  -- Admin: Return all active subjects without filtering
  IF v_is_admin THEN

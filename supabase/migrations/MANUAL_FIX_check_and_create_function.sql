-- Manual fix script to check and create get_user_paper_access_status function
-- Run this in Supabase SQL Editor if migrations are having issues

-- Step 1: Check if function exists
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_proc WHERE proname = 'get_user_paper_access_status'
  ) THEN
    RAISE NOTICE 'Function get_user_paper_access_status does NOT exist. Will create it.';
  ELSE
    RAISE NOTICE 'Function get_user_paper_access_status already exists.';
  END IF;
END $$;
...
FROM pg_proc
WHERE proname = 'get_user_paper_access_status';

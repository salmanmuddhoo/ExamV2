...
serve(async (req)=>{
  ...
  try {
    ...
    const { data: transaction, error: fetchError } = await supabase.from('payment_transactions').select(`
        *,
    ...
    if (transaction.selected_subject_ids && transaction.selected_subject_ids.length > 0) {
      const { data: subjects, error: subjectsError } = await supabase.from('subjects').select('name').in('id', transaction.selected_subject_ids);
    ...
    const userName = transaction.profiles?.first_name ? `${transaction.profiles.first_name} ${transaction.profiles.last_name || ''}`.trim() : 'Valued Customer';
    const receiptData = {
    ...
    // TODO: Change to 'AixamPapers <noreply@yourdomain.com>' after verifying your domain in Resend
    const resendResponse = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from: 'AixamPapers <receipt@aixampapers.com>',
        to: [
          receiptData.userEmail
        ],
